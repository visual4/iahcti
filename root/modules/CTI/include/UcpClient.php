...
class UcpClient
{
	...
	public function receiveCallState($callstate)
	{
		...
		ctilog("Starface debug: starface_user_id = {$this->starface_user_id}");
		ctilog("Starface debug: \$user_id = $user_id");
		if(empty($user_id))
		{
		...
		ctilog("Starface debug: \$callstate = ".print_r($callstate, 1));
		ctilog("Starface debug: \$currentUser->user_name = {$currentUser->user_name}");
