...
// BR: wofür das?
// TODO: an neue Tabelle und RowUpdate anpassen
// perform actions given by post values from the ajax call
if ($_GET && isset($_GET['sfAction']) && isset($_GET['starfaceId'])) {

	$action = strtoupper($_GET['sfAction']);
	$sfID = ($_GET['starfaceId']);
	$query = "
	UPDATE starface_log
	SET
	user_action = '$action'
	...
	$resultSet = $currentUserObj->db->query($query, false);
	if ($currentUserObj->db->checkError()) {
...
// TODO: letzten holen und in PHP prüfen
...
while ($row = $current_user->db->fetchByAssoc($resultSet)) {
	...
	while ($innerRow = $current_user->db->fetchByAssoc($innerResultSet)) {
		...
		if (!empty($innerRow['user_action'])) {
			$userAction['user_action'] = $innerRow['user_action'];
		}
	}

	if (count($userAction) == 0) { // only add Message, if no action by the user was made yet
		...
		if ($phoneToFind) {
			...
			/* $queryAccount = "SELECT
			  ...
			  FROM accounts a
			  left join accounts_contacts ac on (a.id=ac.account_id AND ac.deleted=0)
			  left join contacts c on (ac.contact_id=c.id AND c.deleted=0)
			...
			//$queryLead .= sprintf($sqlReplace, "phone_other", $phoneToFind) . " OR ";
...
foreach ($response as $item) {
	...
	if (!empty($item['company_id'])) {
		...
		$xtpl->parse("call" . $menu_type . ".company_link");
	...
	if (count($item['found']['contacts']) && $item['contact_id'] == $item['found']['contacts'][0]['$contactId']) {
		$xtpl->parse("call" . $menu_type . ".contact_link");
	} elseif (count($item['found']['leads']) && $item['contact_id'] == $item['found']['leads'][0]['$contactId']) {
		$xtpl->parse("call" . $menu_type . ".lead_link");
	}

	$xtpl->assign("CALL_TYPE", $mod_strings[$item['call_type']]);
	if (count($responseArray) < 2)
		$xtpl->parse("call" . $menu_type . ".call_settings");
	if (!empty($item['company_id'])) {
		$xtpl->parse("call" . $menu_type . ".account_exists_show");
	}else
		$xtpl->parse("call" . $menu_type . ".account_add");

	if (!empty($item['contact_id']) && count($item['found']['contacts'])) {
		$xtpl->parse("call" . $menu_type . ".contact_exists_show");
	}else
		$xtpl->parse("call" . $menu_type . ".contact_add");

	if (empty($item['contact_id']) || !count($item['found']['leads'])) {
		$xtpl->parse("call" . $menu_type . ".lead_add");
	} elseif (count($item['found']['leads']) && $item['found']['leads'][0]['$contactId'] == $item['contact_id']) {
		$xtpl->parse("call" . $menu_type . ".lead_exists_show");
	}else
		$xtpl->parse("call" . $menu_type . ".lead_exists");

	$xtpl->parse("call" . $menu_type);
	$item['html'] = str_replace(array("\r", "\t", "\n"), "", /* '<pre>'.nl2br(print_r($item,1)).'</pre>'. */ $xtpl->text("call" . $menu_type));
	unset($item['found']);
